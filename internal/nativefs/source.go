...
//go:embed lock_unix.go lock_other.go profile_linux.go profile_darwin.go open_leaf_unix.go errors.go
...
func Sources() map[string][]byte {
	...
	for _, name := range []string{"errors.go", "lock_unix.go", "lock_other.go", "profile_linux.go", "profile_darwin.go", "open_leaf_unix.go"} {
