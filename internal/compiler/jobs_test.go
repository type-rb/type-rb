...
`),
	...
	if !strings.Contains(string(main.Output), "await trbJobsRunWorkerOrCommand()") || !strings.Contains(string(job.Output), "return (await JOBS_ADAPTER.enqueue(__trbScope, request))") || strings.Contains(string(job.Output), "trbJobsEnqueue(") || strings.Count(string(configuration.Output), "export const JOBS_ADAPTER:") != 1 || strings.Count(string(configuration.Output), "new SQLAdapter(") != 1 || !strings.Contains(string(job.Output), "function perform_in") || !strings.Contains(string(job.Output), "function perform_at") || !strings.Contains(string(runtime.Output), "requestValue.payload_version") || !strings.Contains(string(runtime.Output), "export async function trbJobsClaim") || !strings.Contains(string(runtime.Output), "timestamp.slice(0, 23)") || !strings.Contains(string(runtime.Output), `strftime('%Y-%m-%d %H:%M:%f', 'now')`) || !strings.Contains(string(runtime.Output), `parsed.searchParams.get("allowPublicKeyRetrieval")`) || !strings.Contains(string(runtime.Output), `allowPublicKeyRetrieval: setting === "true"`) {
