...
func (g *generator) jobsClassEnqueueMethods(manifest *jobs.Manifest) {
	for _, job := range manifest.Jobs {
		...
		g.indent++
		g.line("export async function " + tsMethodName("perform_later") + "(" + strings.Join(parameters, ", ") + "): Promise<__trbJobResult.Result<JobReference, EnqueueError>> {")
		g.indent++
		g.line("try {")
		g.indent++
...
func (g *generator) jobsStorage(config jobssql.Config) {
	...
	release := "UPDATE trb_jobs SET state = 'ready', claimed_by = NULL, claimed_at = NULL, updated_at = CURRENT_TIMESTAMP WHERE state = 'running' AND claimed_by = " + tsJobsPlaceholder(config.DatabaseAdapter, 1)
	...
	retry := "UPDATE trb_jobs SET state = 'ready', attempts = 0, run_at = CURRENT_TIMESTAMP, claimed_by = NULL, claimed_at = NULL, last_error = NULL, updated_at = CURRENT_TIMESTAMP WHERE id = " + tsJobsPlaceholder(config.DatabaseAdapter, 1) + " AND state = 'failed'"
	...
	discard := "DELETE FROM trb_jobs WHERE id = " + tsJobsPlaceholder(config.DatabaseAdapter, 1) + " AND state != 'running'"
