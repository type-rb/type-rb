...
func (g *generator) intrinsic(name string, call *ir.Call, arguments []string) string {
	...
	case "trb.std.file.read", "trb.std.file.read_text":
		...
		failure := g.filesystemNativeError(operation, path, caught)
		...
		if name == "trb.std.file.read_text" {
			invalidEncoding := g.filesystemErrorValue(errorType, operation, path, strconv.Quote("file is not valid UTF-8"), "InvalidEncoding")
			value = "decoded"
			failureCheck = "let decoded: string; try { decoded = new TextDecoder(\"utf-8\", { fatal: true, ignoreBOM: true }).decode(" + data + "); } catch { return " + resultError(invalidEncoding) + "; } "
		}
		return "((): " + resultType + " => { const " + handle + " = " + arguments[0] + "; const " + limit + " = " + arguments[1] + "; const " + path + " = " + handle + ".path; if (" + limit + " < 0) { return " + resultError(invalid) + "; } const " + filesystem + " = (globalThis as any).process?.getBuiltinModule?.(\"fs\"); if (" + filesystem + " === undefined) { return " + resultError(unavailable) + "; } try { const " + buffer + " = new Uint8Array(65536); const " + chunks + ": Array<Uint8Array> = []; let " + count + " = 0; while (" + count + " <= " + limit + ") { const " + remaining + " = " + limit + " - " + count + "; const " + request + " = " + remaining + " === 0 ? 1 : Math.min(" + buffer + ".byteLength, " + remaining + "); const " + readCount + " = " + filesystem + ".readSync(" + handle + ".fd, " + buffer + ", 0, " + request + ", null); if (" + readCount + " === 0) break; " + chunks + ".push(" + buffer + ".slice(0, " + readCount + ")); " + count + " += " + readCount + "; } if (" + count + " > " + limit + ") { return " + resultError(tooLarge) + "; } const " + data + " = new Uint8Array(" + count + "); let " + offset + " = 0; for (const " + chunk + " of " + chunks + ") { " + data + ".set(" + chunk + ", " + offset + "); " + offset + " += " + chunk + ".byteLength; } " + failureCheck + "return " + filesystemOK(value) + "; } catch (" + caught + ") { const " + message + " = " + caught + " instanceof Error ? " + caught + ".message : String(" + caught + "); return " + resultError(failure) + "; } })()"
	...
	case "trb.std.arrays.concat":
		return "((values: " + callType + ", other: " + callType + "): " + callType + " => [...values, ...other])(" + arguments[0] + ", " + arguments[1] + ")"
	...
	case "trb.std.hashes.merge":
		return "((values: " + callType + ", incoming: " + callType + "): " + callType + " => ({ ...values, ...incoming }))(" + arguments[0] + ", " + arguments[1] + ")"
...
func tsWebContextWith(call *ir.Call, arguments []string) string {
	...
	return "((): " + tsType(call.ExprType()) + " => { const contextValue = " + arguments[0] + "; const contextKey = " + arguments[1] + "; const existing = (contextValue as any).__trb__trb_context_state; const contextState = existing instanceof Map ? new Map<unknown, unknown>(existing) : new Map<unknown, unknown>(); contextState.set(contextKey, " + arguments[2] + "); const result = contextValue.with_request(contextValue.__trb_request); (result as any).__trb__trb_context_state = contextState; return result; })()"
}
...
func tsWebContextWithRequest(call *ir.Call, arguments []string) string {
	...
	return "((): " + tsType(call.ExprType()) + " => { const contextValue = " + arguments[0] + "; const result = contextValue.with_request(" + arguments[1] + "); (result as any).__trb__trb_context_state = (contextValue as any).__trb__trb_context_state; return result; })()"
}
...
func (g *generator) tsWebContextFetch(call *ir.Call, arguments []string) string {
	...
	return "((): " + resultType + " => { const contextValue = " + arguments[0] + "; const contextKey = " + arguments[1] + "; const contextState = (contextValue as any).__trb__trb_context_state; if (contextState instanceof Map && contextState.has(contextKey)) return " + result + ".Ok<" + valueType + ", " + errorType + ">(contextState.get(contextKey) as " + valueType + "); return " + result + ".Err<" + valueType + ", " + errorType + ">({ key: contextKey.__trb_name }); })()"
}
