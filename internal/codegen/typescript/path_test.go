...
func TestHostPathCompositionBothGrammars(t *testing.T) {
	...
	for _, windows := range []bool{false, true} {
		for _, fixture := range pathfixture.Cases {
			...
			source.WriteString("console.log(" + pathJoinExpression(string(parent), string(child), strconv.FormatBool(windows)) + ");\n")
			expected.WriteString(fixture.Expected(windows) + "\n")
		}
	}
	output, err := exec.Command("bun", "-e", source.String()).CombinedOutput()
