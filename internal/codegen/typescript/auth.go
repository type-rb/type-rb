...
func (g *generator) oidcRuntimeSupport() {
	...
	g.line(`type TrbOidcProviderMetadata = Readonly<{ issuer: string; authorization_endpoint?: string; token_endpoint?: string; jwks_uri: string; end_session_endpoint?: string }>;`)
	g.line(`const trbOidcProviderCache = new Map<string, Readonly<{ metadata: TrbOidcProviderMetadata; expires: number }>>();`)
	g.line(`const trbOidcJwksCache = new Map<string, Readonly<{ keys: TrbOidcJwk[]; expires: number; refreshAfter?: number }>>();`)
	...
	g.indent--
	g.line(`}`)
	g.line(`async function trbOidcLoadProvider(issuer: string): Promise<Readonly<{ metadata?: TrbOidcProviderMetadata; failure?: TrbOidcFailure }>> {`)
	g.indent++
	g.line(`if (issuer === "") return { failure: { kind: "provider", message: "OIDC issuer is empty" } }; const cached = trbOidcProviderCache.get(issuer); if (cached !== undefined && cached.expires > Date.now()) return { metadata: cached.metadata };`)
	g.line(`try { const uri = issuer.replace(/\/+$/, "") + "/.well-known/openid-configuration"; const response = await fetch(uri, { signal: AbortSignal.timeout(5000) }); if (!response.ok) return { failure: { kind: "provider", message: ` + "`load OIDC discovery: HTTP ${response.status}`" + ` } }; const text = await response.text(); if (text.length > 1_048_576) return { failure: { kind: "provider", message: "OIDC discovery response is too large" } }; const metadata = JSON.parse(text) as TrbOidcProviderMetadata; if (metadata.issuer !== issuer || typeof metadata.jwks_uri !== "string" || metadata.jwks_uri === "") return { failure: { kind: "provider", message: "OIDC discovery response is invalid" } }; trbOidcProviderCache.set(issuer, { metadata, expires: Date.now() + 300_000 }); return { metadata }; } catch (error) { return { failure: { kind: "provider", message: error instanceof Error ? error.message : String(error) } }; }`)
	g.indent--
	g.line(`}`)
	g.line(`async function trbOidcVerifyBearer(headers: Record<string, string[]>, issuer: string, audience: string, jwksUri: string | null, rolesClaim: string): Promise<Readonly<{ data?: TrbOidcPrincipalData; failure?: TrbOidcFailure }>> {`)
	g.indent++
	...
	g.indent--
	g.line(`}`)
	g.line(`async function trbOidcVerifyJwt(token: string, issuer: string, audience: string, jwksUri: string | null, rolesClaim: string, nonce: string): Promise<Readonly<{ data?: TrbOidcPrincipalData; failure?: TrbOidcFailure }>> {`)
	g.indent++
	g.line(`try {`)
	g.indent++
	g.line(`if (jwksUri === null) { const provider = await trbOidcLoadProvider(issuer); if (provider.failure !== undefined) return { failure: provider.failure }; jwksUri = provider.metadata!.jwks_uri; }`)
	...
	g.line(`let loaded = await trbOidcLoadJwks(jwksUri, false); if (loaded.failure !== undefined) return { failure: loaded.failure };`)
	g.line(`let jwk = loaded.keys!.find((candidate) => candidate.kid === header.kid && candidate.kty === "RSA" && (candidate.use === undefined || candidate.use === "sig") && (candidate.alg === undefined || candidate.alg === "RS256"));`)
	g.line(`if (jwk === undefined) { loaded = await trbOidcLoadJwks(jwksUri, true); if (loaded.failure !== undefined) return { failure: loaded.failure }; jwk = loaded.keys!.find((candidate) => candidate.kid === header.kid && candidate.kty === "RSA" && (candidate.use === undefined || candidate.use === "sig") && (candidate.alg === undefined || candidate.alg === "RS256")); }`)
	...
	g.indent--
	g.line(`}`)
	g.line(`async function trbOidcLoadJwks(uri: string, force: boolean): Promise<Readonly<{ keys?: TrbOidcJwk[]; failure?: TrbOidcFailure }>> {`)
	g.indent++
	g.line(`if (uri === "") return { failure: { kind: "provider", message: "JWKS URI is empty" } }; const cached = trbOidcJwksCache.get(uri); if (!force && cached !== undefined && cached.expires > Date.now()) return { keys: cached.keys }; if (force && cached?.refreshAfter !== undefined && cached.refreshAfter > Date.now()) return { keys: cached.keys }; if (force && cached !== undefined) trbOidcJwksCache.set(uri, { ...cached, refreshAfter: Date.now() + 30_000 });`)
	g.line(`try { const response = await fetch(uri, { signal: AbortSignal.timeout(5000) }); if (!response.ok) return { failure: { kind: "provider", message: ` + "`load JWKS: HTTP ${response.status}`" + ` } }; const text = await response.text(); if (text.length > 1_048_576) return { failure: { kind: "provider", message: "JWKS response is too large" } }; const document = JSON.parse(text) as { keys?: TrbOidcJwk[] }; if (!Array.isArray(document.keys) || document.keys.length === 0) return { failure: { kind: "provider", message: "JWKS response is invalid" } }; trbOidcJwksCache.set(uri, { keys: document.keys, expires: Date.now() + 300_000, refreshAfter: trbOidcJwksCache.get(uri)?.refreshAfter }); return { keys: document.keys }; } catch (error) { return { failure: { kind: "provider", message: error instanceof Error ? error.message : String(error) } }; }`)
	g.indent--
	g.line(`}`)
...
type TrbOidcSessionOptions = Readonly<{ issuer: string; client_id: string; client_secret: string; authorization_endpoint: string | null; token_endpoint: string | null; jwks_uri: string | null; redirect_uri: string; post_logout_redirect_uri: string; end_session_endpoint: string | null; scope: string; audience: string | null; roles_claim: string; cookie_name: string; cookie_secret: string; secure: boolean }>;
