...
func isSuspendingIntrinsic(intrinsic string, fails types.Type) bool {
	return intrinsic == "trb.jobs.perform_later" || isSuspendingORM(intrinsic, fails) || intrinsic == "trb.web.testing.dispatch" || intrinsic == "trb.web.middleware.logger.call" || intrinsic == "trb.web.middleware.timeout.call" || intrinsic == "trb.platform.typescript.browser.request"
}
