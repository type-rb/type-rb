...
func (g *generator) authIntrinsic(name string, call *ir.Call, arguments []string) (string, bool) {
	...
	verification := "trbOidcVerifyBearer(" + context + ".Request.Headers, " + options + ".Issuer, " + options + ".Audience, " + nullableGoString(options+".JwksUri") + ", " + options + ".RolesClaim)"
...
func (g *generator) sessionAuthIntrinsic(name string, call *ir.Call, arguments []string, context, options string) string {
	...
	case "trb.web.auth.session.start_login":
		return "trbOidcStartLogin(" + context + ".Request, " + options + ".Issuer, " + nullableGoString(options+".AuthorizationEndpoint") + ", " + options + ".ClientId, " + options + ".RedirectUri, " + options + ".Scope, " + nullableGoString(options+".Audience") + ", " + options + ".CookieName, " + options + ".CookieSecret, " + boolString(options+".Secure") + ", " + arguments[2] + ")"
	case "trb.web.auth.session.complete_login":
		return "trbOidcCompleteLogin(" + base + ".Issuer, " + options + ".ClientId, " + options + ".ClientSecret, " + options + ".RedirectUri, " + nullableGoString(options+".TokenEndpoint") + ", " + nullableGoString(options+".JwksUri") + ", " + options + ".RolesClaim, " + options + ".CookieName, " + options + ".CookieSecret, " + boolString(options+".Secure") + ")"
	case "trb.web.auth.session.end_session":
		return "trbOidcEndSession(" + context + ".Request, " + options + ".Issuer, " + options + ".CookieName, " + options + ".CookieSecret, " + boolString(options+".Secure") + ", " + nullableGoString(options+".EndSessionEndpoint") + ", " + options + ".PostLogoutRedirectUri, " + options + ".ClientId)"
	}
...
func (g *generator) oidcRuntimeSupport() {
	...
	g.line(`type trbOidcProviderMetadata struct { Issuer string ` + "`json:\"issuer\"`" + `; AuthorizationEndpoint string ` + "`json:\"authorization_endpoint\"`" + `; TokenEndpoint string ` + "`json:\"token_endpoint\"`" + `; JwksURI string ` + "`json:\"jwks_uri\"`" + `; EndSessionEndpoint string ` + "`json:\"end_session_endpoint\"`" + ` }`)
	g.line(`type trbOidcProviderCacheEntry struct { metadata trbOidcProviderMetadata; expires time.Time }`)
	g.line(`type trbOidcJwksCacheEntry struct { keys []trbOidcJwk; expires time.Time; refreshAfter time.Time }`)
	...
	g.indent++
	g.line(`if issuer == "" { return trbOidcProviderMetadata{}, &trbOidcFailure{kind: "provider", message: "OIDC issuer is empty"} }; trbOidcProviderCache.Lock(); cached, ok := trbOidcProviderCache.entries[issuer]; trbOidcProviderCache.Unlock(); if ok && time.Now().Before(cached.expires) { return cached.metadata, nil }`)
	g.line(`uri := strings.TrimRight(issuer, "/") + "/.well-known/openid-configuration"; response, err := (&http.Client{Timeout: 5 * time.Second}).Get(uri); if err != nil { return trbOidcProviderMetadata{}, &trbOidcFailure{kind: "provider", message: fmt.Sprintf("load OIDC discovery: %v", err)} }; defer response.Body.Close(); if response.StatusCode != http.StatusOK { return trbOidcProviderMetadata{}, &trbOidcFailure{kind: "provider", message: fmt.Sprintf("load OIDC discovery: HTTP %d", response.StatusCode)} }; body, err := io.ReadAll(io.LimitReader(response.Body, (1<<20)+1)); if err != nil { return trbOidcProviderMetadata{}, &trbOidcFailure{kind: "provider", message: fmt.Sprintf("read OIDC discovery: %v", err)} }; if len(body) > 1<<20 { return trbOidcProviderMetadata{}, &trbOidcFailure{kind: "provider", message: "OIDC discovery response is too large"} }`)
	g.line(`var metadata trbOidcProviderMetadata; if json.Unmarshal(body, &metadata) != nil || metadata.Issuer != issuer || metadata.JwksURI == "" { return trbOidcProviderMetadata{}, &trbOidcFailure{kind: "provider", message: "OIDC discovery response is invalid"} }; trbOidcProviderCache.Lock(); trbOidcProviderCache.entries[issuer] = trbOidcProviderCacheEntry{metadata: metadata, expires: time.Now().Add(5 * time.Minute)}; trbOidcProviderCache.Unlock(); return metadata, nil`)
	g.indent--
	g.line(`}`)
	...
	g.line(`func trbOidcSelectJwk(keys []trbOidcJwk, kid string) *trbOidcJwk { for index := range keys { if keys[index].Kid == kid && keys[index].Kty == "RSA" && (keys[index].Use == "" || keys[index].Use == "sig") && (keys[index].Alg == "" || keys[index].Alg == "RS256") { return &keys[index] } }; return nil }`)
	g.line(`func trbOidcLoadJwks(uri string, force bool) ([]trbOidcJwk, *trbOidcFailure) {`)
	g.indent++
	g.line(`if uri == "" { return nil, &trbOidcFailure{kind: "provider", message: "JWKS URI is empty"} }; now := time.Now(); trbOidcJwksCache.Lock(); cached, ok := trbOidcJwksCache.entries[uri]; if !force && ok && now.Before(cached.expires) { trbOidcJwksCache.Unlock(); return cached.keys, nil }; if force && ok && now.Before(cached.refreshAfter) { trbOidcJwksCache.Unlock(); return cached.keys, nil }; if force && ok { cached.refreshAfter = now.Add(30 * time.Second); trbOidcJwksCache.entries[uri] = cached }; trbOidcJwksCache.Unlock()`)
	...
	g.line(`var document struct { Keys []trbOidcJwk ` + "`json:\"keys\"`" + ` }; if json.Unmarshal(body, &document) != nil || len(document.Keys) == 0 { return nil, &trbOidcFailure{kind: "provider", message: "JWKS response is invalid"} }; trbOidcJwksCache.Lock(); refreshAfter := time.Time{}; if existing, found := trbOidcJwksCache.entries[uri]; found { refreshAfter = existing.refreshAfter }; trbOidcJwksCache.entries[uri] = trbOidcJwksCacheEntry{keys: document.Keys, expires: time.Now().Add(5 * time.Minute), refreshAfter: refreshAfter}; trbOidcJwksCache.Unlock(); return document.Keys, nil`)
	g.indent--
	g.line(`}`)
