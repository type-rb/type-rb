...
func (g *generator) intrinsic(name string, call *ir.Call, arguments []string) string {
	...
	case "trb.std.url.parse_query":
		...
		g.requireImport("unicode/utf8", "utf8")
		resultType, _, _ := filesystemResultType()
		parameterType := g.goType(types.FromName("QueryParameter"))
		...
		return "func() " + resultType + " { input := " + arguments[0] + "; parameters := make([]" + parameterType + ", 0); decode := func(component string) (string, bool, bool) { characters := []rune(component); value := make([]byte, 0, len(component)); hexadecimal := func(character rune) (byte, bool) { switch { case character >= '0' && character <= '9': return byte(character - '0'), true; case character >= 'A' && character <= 'F': return byte(character - 'A' + 10), true; case character >= 'a' && character <= 'f': return byte(character - 'a' + 10), true; default: return 0, false } }; for index := 0; index < len(characters); index++ { character := characters[index]; if character == '+' { value = append(value, ' '); continue }; if character != '%' { value = append(value, []byte(string(character))...); continue }; if index+2 >= len(characters) { return \"\", false, true }; high, highOK := hexadecimal(characters[index+1]); low, lowOK := hexadecimal(characters[index+2]); if !highOK || !lowOK { return \"\", false, true }; value = append(value, high<<4|low); index += 2 }; if !utf8.Valid(value) { return \"\", true, false }; return string(value), true, true }; for _, part := range strings.Split(input, \"&\") { if part == \"\" { continue }; namePart, valuePart := part, \"\"; if separator := strings.IndexByte(part, '='); separator >= 0 { namePart, valuePart = part[:separator], part[separator+1:] }; name, escapeOK, utf8OK := decode(namePart); if !escapeOK { return " + invalidNameEscape + " }; if !utf8OK { return " + invalidNameUtf8 + " }; value, escapeOK, utf8OK := decode(valuePart); if !escapeOK { return " + invalidValueEscape + " }; if !utf8OK { return " + invalidValueUtf8 + " }; parameters = append(parameters, " + parameterType + "{Name: name, Value: value}) }; return " + filesystemOK("parameters") + " }()"
	case "trb.std.url.build_query":
		g.requireImport("strings", "")
		return "func(parameters " + g.goType(call.Arguments[0].Value.ExprType()) + ") string { const hexadecimal = \"0123456789ABCDEF\"; encode := func(component string) string { var builder strings.Builder; for _, octet := range []byte(component) { unreserved := octet >= 'A' && octet <= 'Z' || octet >= 'a' && octet <= 'z' || octet >= '0' && octet <= '9' || octet == '*' || octet == '-' || octet == '.' || octet == '_'; if octet == ' ' { builder.WriteByte('+') } else if unreserved { builder.WriteByte(octet) } else { builder.WriteByte('%'); builder.WriteByte(hexadecimal[octet>>4]); builder.WriteByte(hexadecimal[octet&15]) } }; return builder.String() }; var builder strings.Builder; for index, parameter := range parameters { if index > 0 { builder.WriteByte('&') }; builder.WriteString(encode(parameter.Name)); builder.WriteByte('='); builder.WriteString(encode(parameter.Value)) }; return builder.String() }(" + arguments[0] + ")"
