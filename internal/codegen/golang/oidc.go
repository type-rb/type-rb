...
func (g *generator) oidcIntrinsic(name string, call *ir.Call, arguments []string) (string, bool) {
	...
	request := arguments[0]
	options := arguments[1]
	return "func() " + resultType + " { requestValue := " + request + "; optionsValue := " + options + "; authorization := []string{}; for _, header := range requestValue.TrbFieldHeaders.Entries() { if strings.EqualFold(header.Name, \"authorization\") { authorization = append(authorization, header.Value) } }; jwksURI := \"\"; if optionsValue.JwksUri != nil { jwksURI = *optionsValue.JwksUri }; data, failure := trbOidcVerifyBearer(authorization, optionsValue.Issuer, optionsValue.Audience, jwksURI, optionsValue.RolesClaim, optionsValue.ClockSkewSeconds); if failure != nil { errorKind := " + kind("InvalidCredentials") + "; if failure.kind == \"missing\" { errorKind = " + kind("MissingCredentials") + " } else if failure.kind == \"provider\" { errorKind = " + kind("Provider") + " } else if failure.kind == \"configuration\" { errorKind = " + kind("Configuration") + " }; return " + resultAlias + ".NewResultErr[" + principalType + ", " + errorType + "](" + errorType + "{Kind: errorKind, Message: failure.message}) }; var displayName *string; if data.name != \"\" { value := data.name; displayName = &value }; var email *string; if data.email != \"\" { value := data.email; email = &value }; principal := " + principalType + "{Subject: data.subject, Name: displayName, Email: email, Roles: data.roles}; return " + resultAlias + ".NewResultOk[" + principalType + ", " + errorType + "](principal) }()", true
}
