...
func (g *generator) cliRun(call *ir.Call) string {
	...
	for _, argument := range call.Arguments {
		if argument.Name != "" {
			arguments[argument.Name] = g.expr(argument.Value)
		}
	}
	name := arguments["name"]
	...
	return "trbCliRun" + strconv.Itoa(index) + "(" + strings.Join([]string{name, version, about}, ", ") + ")"
}
...
func (g *generator) cliInvocation(index int, invocation *cliapp.Invocation) {
	...
	g.line("func trbCliRun" + strconv.Itoa(index) + "(name string, version *string, about *string) " + resultType + " {")
	g.indent++
...
func (g *generator) cliEnumValue(command cliapp.Command, commandIndex int) string {
	...
	values := g.cliFieldValues(*command.Payload, prefix, command.Name)
	...
	for index, command := range schema.Commands {
		...
		if command.Payload != nil {
			...
			for fieldIndex, field := range command.Payload.Fields {
				commandFields[fieldIndex] = cliFieldLiteral(command.Name+"."+field.Name, field)
			}
		}
...
func (g *generator) cliRuntimeSupport() {
	...
	g.line("if result.Command == \"\" && len(spec.Commands) > 0 && !strings.HasPrefix(argument, \"-\") {")
	g.indent++
...
func (g *generator) cliRuntimeHelpers() {
	...
	g.line("if len(fields) > 0 { fmt.Fprint(os.Stdout, \" [OPTIONS]\") }")
	...
	g.line("options := false; for _, field := range fields { options = options || !field.Positional }")
