...
func (g *generator) filesystemStructuredBlock(block *ir.StructuredBlock) {
	...
	g.indent++
	g.line("return Result::Err.new("+rubyFilesystemError("open", path, openError+".message", "Other")+")", "")
	g.indent--
	g.line("end", "")
	...
	g.indent++
	g.line(result+" = Result::Err.new("+rubyFilesystemError("close", path, closeError+".message", "Other")+") if "+completed, "")
	g.indent--
	g.line("end", "")
	g.indent--
	g.line("end", "")
