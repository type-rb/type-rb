...
func (c *CLI) runWeb(args []string) error {
	if len(args) == 0 {
		return errors.New("web requires a subcommand: openapi")
	}
	switch args[0] {
	case "openapi":
		return c.runWebOpenAPI(args[1:])
	default:
		return fmt.Errorf("unknown web command %q; expected openapi", args[0])
	}
}
...
func (c *CLI) runWebOpenAPI(args []string) error {
	...
	if snapshot.HasErrors() {
		return compiler.NewCompileError("", diagnostic.ProjectIntegration, snapshot.Diagnostics)
	}
	...
	if manifest == nil {
		return errors.New("web openapi requires a trb/web file-routing project")
	}
	...
	if err != nil {
		return fmt.Errorf("build trb/web OpenAPI declaration input: %w", err)
	}
	...
	document, issues, err := webintegration.BuildOpenAPI(manifest.EndpointCatalog, declarations, webintegration.OpenAPIOptions{
	...
	if len(issues) != 0 {
		...
		for _, issue := range issues {
			path := moduleFiles[issue.ModulePath]
			if path == "" {
				path = config.Path
			}
			diagnostics = append(diagnostics, diagnostic.Diagnostic{
				Code: diagnostic.ProjectIntegration, Severity: diagnostic.Error, Message: issue.Message,
				Path: path, Span: packageextensionhost.ImportSourceSpan(issue.Span),
			})
		}
		return compiler.NewCompileError(config.Path, diagnostic.ProjectIntegration, diagnostics)
	}
