...
`
	...
	if jobName != "SendReceiptJob" || payload != `[42,"ada@example.test"]` || queueName != "mail" || priority != 10 || maximumAttempts != 5 || state != "ready" {
		t.Fatalf("unexpected Ruby persisted job name=%q payload=%q queue=%q priority=%d maximum_attempts=%d state=%q", jobName, payload, queueName, priority, maximumAttempts, state)
	}
