...
func (c catalog) schema(rootReference TypeReference) (Schema, []Issue) {
	...
	for index, field := range root.Fields {
		...
		if metadata.kind == "subcommand" {
			for _, key := range unsupportedMetadata(metadata, nil) {
...
func (c catalog) commands(reference TypeReference, enum packageextension.ProjectEnum) ([]Command, []Issue) {
	...
	for _, member := range enum.Members {
		...
		issues = append(issues, metadataIssues...)
		for _, key := range unsupportedMetadata(metadata, map[string]bool{"name": true, "about": true}) {
			issues = append(issues, Issue{Message: fmt.Sprintf("enum member @cli does not support %s metadata", key), Span: member.Span})
		}
		if metadata.kind != "" {
		...
		case 0:
		case 1:
			command.PayloadName = member.Parameters[0].Name
			command.PayloadNamedOnly = member.Parameters[0].NamedOnly
			...
			for index, field := range record.Fields {
				...
				issues = append(issues, issuesInModule(fieldIssues, payloadReference.ModulePath)...)
			}
...
func issuesInModule(issues []Issue, modulePath string) []Issue {
	for index := range issues {
		if issues[index].ModulePath == "" {
			issues[index].ModulePath = modulePath
		}
	}
	return issues
}
...
func cliMetadata(attributes []packageextension.ProjectAttribute, span packageextension.SourceSpan) (metadata, []Issue) {
	result := metadata{keys: map[string]bool{}}
	var issues []Issue
	seen := map[string]bool{}
	for _, attribute := range attributes {
		...
		for _, argument := range attribute.Arguments {
			...
			if seen[key] {
				issues = append(issues, Issue{Message: fmt.Sprintf("duplicate @cli metadata %s", key), Span: argument.Span})
				continue
			}
			seen[key] = true
			...
			case "name", "long":
				result.name = value
				result.keys[argument.Name] = true
			case "short":
				result.short = value
				result.keys[argument.Name] = true
			case "about":
...
func unsupportedMetadata(metadata metadata, allowed map[string]bool) []string {
	var result []string
	for key := range metadata.keys {
		if !allowed[key] {
...
func scalarField(modulePath string, field packageextension.ProjectRecordField, metadata metadata, order int) (Field, []Issue) {
	result := Field{
		...
		Nullable: field.Type.Resolved.Nullable, ModulePath: modulePath, SourceOrder: order, Span: field.Span,
	}
	if metadata.kind == "" {
	...
	if result.Positional {
		if unsupported := unsupportedMetadata(metadata, map[string]bool{"about": true, "value_name": true}); len(unsupported) > 0 {
			return result, []Issue{{Message: fmt.Sprintf("positional record field %s does not support @cli metadata %s", field.Name, strings.Join(unsupported, ", ")), Span: field.Span}}
		}
	}
...
func kebab(value string) string {
	...
	for index, r := range runes {
		...
		if index > 0 && unicode.IsUpper(r) {
			...
			if previous != '_' && previous != '-' && (unicode.IsLower(previous) || unicode.IsDigit(previous) || unicode.IsUpper(previous) && nextLower) {
				result.WriteByte('-')
			}
		}
		if r == '-' {
