...
Direct Range iteration streams in Go and TypeScript output and in the typed-IR
...
across all backends and the REPL. Sequential transformations, predicates,
searches and keyed sorting use that same live traversal, including suspending
TypeScript blocks. Batching retains the requested size and owns
...
`newtype` declarations over concrete non-nullable representations with optional
class/instance methods and private raw construction over recursively immutable
values, explicit
