...
- Evolve the initial `trb/cli` single-binary schema from application evidence.
  Consider environment fallback, validation constraints, aliases, completion,
  and nested subcommands only as extensions to the shared record and
  payload-enum model; keep dynamic plugins and Ruby/TypeScript launcher
  generation outside its single-binary direction.
