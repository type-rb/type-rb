...
The value boundary accepts `String`, `Integer`, `Float`, and `Boolean`. An
option may wrap one of those scalar types in `Array`; each occurrence appends
one converted value in command-line order. Repeated Arrays are not positional
arguments, and existing non-Array options retain final-occurrence behavior.
Required and default behavior continues to come from the record field.
