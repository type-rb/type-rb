...
Static command-line schemas need to distinguish positional fields, options,
and subcommands without adding a second data-model declaration. They also need
to omit a parsed field so the data type, rather than the CLI package, owns its
default value. The same omission semantics are useful outside command-line
applications.
