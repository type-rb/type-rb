...
language wrapper. Unbounded one-shot reads are unsuitable when input size is
controlled externally, and a one-shot write does not provide a scoped place to
express exclusive create-new.
...
one byte beyond the bound.
`file.write` accepts `Bytes`, while `file.write_text` accepts `String`. Writes
do not imply locking, flushing, or durable storage.
...
positionals in this slice, so their termination rule remains unambiguous. When
a schema declares subcommands, callers must still select one explicitly;
repeated options do not introduce a default subcommand.

The exact named import `fail(message)` from `trb/cli` unwinds scoped cleanup,
writes one application diagnostic to standard error, and exits with status 1.
Parser failures retain status 2. It is a terminal single-binary application
operation, not a portable process-termination API.
...
general path parsing or joining, variadic positionals, shell completion, nested
subcommands, default root commands, or Ruby and TypeScript CLI launcher
generation.
...
- Existing scalar CLI options retain their final-occurrence behavior.
